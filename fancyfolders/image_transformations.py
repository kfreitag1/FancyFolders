...
def generate_folder_icon(folder_style: FolderStyle = FolderStyle.big_sur_light,
                         ...
                         preview_size=None, icon_scale=1.0, tint_colour=None, text=None,
                         font_style=SFFont.heavy, image=None):
    ...
    bounding_box = tuple(int(size * percent)
                         for percent in bounding_box_percentages)
...
def _generate_mask_from_text(text, image_size, font_style=SFFont.heavy):
    ...
    font_path = get_font_location(
        font_style.filename(), include_internal=False)
    if font_path is None:
        font_path = get_first_font_installed(
...
def _resize_image_in_box(image: Image, box):
    ...
    downscale_ratio = min(
        box_size[0] / image.size[0], box_size[1] / image.size[1])
    scaled_image = image.resize(
        (int(image.width * downscale_ratio), int(image.height * downscale_ratio)))
...
def scaled_box(box, scale, max_size):
    ...
    bottom_offset = (int(
        (bottom_point[0] - center[0])*scale), int((bottom_point[1] - center[1])*scale))
